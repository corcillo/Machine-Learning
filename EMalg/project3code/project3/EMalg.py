...
#REAL COUNTS EVALUATED DIRECTLY FROM LABELED DATA
    ...
    # Your code here
    #pt it a column vector of tag probabilities
    #t is a column vector of real tag numbers
    #et is a column vecotr of expected tag numbers
    #the number of tags = c*t + et, row corresponds to number of tag[row]
    pt = c*t + et
    for num in pt: #divide the number of a certain tag by the total number of tags to get its probaility
        num/=np.sum(pt)
    
#ptw is a matrix of TxW that whose ith,jth entry is the probability of a word j  given the tag i
#ptw[i,j] = ntw[i,j]/total number of       
    ptw= c*tw + etw  #finds the number of i tags on a j word
    for row in ptw:
        for col in row:
            col/=np.sum(row)
...
def predictA(wordList, pt, ptw, ptpw, ptnw):
    ...
    # ptpw: p_{-1}(w|t)
    # ptnw: p_{+1}(w|t)
    #arg max over joint prob dist for each word
...
def predictB(wordList, pt, ptw, ptpw, ptnw):
    ...
    # pred is the lis+t of prediction, each element is a list of tag index predictions for each word in the sentence
    # e.g. pred = [[1,2], [2,3]]
    pred = []
